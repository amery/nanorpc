...
- **Decoupled Architecture**: Clean separation of concerns for better testability
- **Ping-Pong Protocol**: Built-in health check and connection validation
- **Graceful Shutdown**: Proper session cleanup and resource management
...
- **Comprehensive Testing**: 86.4% test coverage with unit and integration tests
...
func (h *CustomHandler) HandleMessage(ctx context.Context, session server.Session, req *nanorpc.NanoRPCRequest) error {
...
func (h *CustomHandler) handleRequest(session server.Session, req *nanorpc.NanoRPCRequest) error {
    ...
    // Send response (requires casting to access Write method)
    if sessionWriter, ok := session.(*server.DefaultSession); ok {
