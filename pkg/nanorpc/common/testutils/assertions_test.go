...
// TestContainsAt tests the internal containsAt function
func TestContainsAt(t *testing.T) {
	...
	for _, tt := range tests {
		result := containsAt(tt.str, tt.substr)
		if result != tt.expected {
			t.Errorf("containsAt(%q, %q) = %v, want %v", tt.str, tt.substr, result, tt.expected)
		}
	}
}
